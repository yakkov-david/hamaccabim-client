...
interface AnalyticsData {
    event: string;
    timestamp: string;
    landingPageId?: string; // Optional landingPageId field
}
...
const AnalyticsPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchAnalyticsData = async () => {
            try {
                if (config.apiKey && landingPageId) { // Ensure the API key and landingPageId are defined
                    const headers = new Headers();
                    headers.set('Content-Type', 'application/json');
                    headers.set('x-api-key', config.apiKey);

                    const response = await fetch(`${config.apiUrl}/analytics/${landingPageId}`, {
                        headers: headers,
                    });
                    ...
                    if (response.ok) {
                        ...
                        if (data) {
                            const result:AnalyticsData[] = data.result
                            const visitCount = result.filter(item => item.event === 'visit').length;
                            const clickCount = result.filter(item => item.event === 'click').length;
                    ...
                    throw new Error('API key or landingPageId is missing');
                }
            } catch (error) {
